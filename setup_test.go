...
func TestSetup(t *testing.T) {
	tests := []struct {
		...
		{name: "minimal-config", input: corefileUDPLocal, expectedFrom: ".", expectedAttempts: 3, expectedWorkers: 1, expectedTimeout: defaultTimeout, expectedNetwork: UDP, expectedRace: false, expectedRaceContinueOnErrorResponse: false, expectedServerCount: 1, expectedLoadFactor: nil, expectedPolicy: ""},
		...
		{name: "two-hosts-tcp", input: "fanout . 127.0.0.1 127.0.0.2 {\nnetwork tcp\n}", expectedFrom: ".", expectedTimeout: defaultTimeout, expectedAttempts: 3, expectedWorkers: 2, expectedNetwork: TCP, expectedRace: false, expectedRaceContinueOnErrorResponse: false, expectedTo: []string{localDNS53, localDNS53Alt}, expectedServerCount: 2, expectedLoadFactor: nil, expectedPolicy: ""},
...
func TestOnShutdown(t *testing.T) {
	f := New()
	c1 := &closableClient{addr: localDNS53}
